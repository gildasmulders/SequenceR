...
def build_save_vocab(train_dataset, fields, opt):
    ...
    if len(opt.numerical_feat_names) > 0:
        opt.numerical_feat_names = opt.numerical_feat_names[1:-1].split(', ')
        if len(opt.numerical_feat_names)==1 and len(opt.numerical_feat_names[0])==0:
            opt.numerical_feat_names=[]

    fields = inputters.build_vocab(
        train_dataset, fields, opt.data_type, opt.share_vocab,
        opt.src_vocab, opt.src_vocab_size, opt.src_words_min_frequency,
        opt.tgt_vocab, opt.tgt_vocab_size, opt.tgt_words_min_frequency, opt.numerical_feat_names
    )
