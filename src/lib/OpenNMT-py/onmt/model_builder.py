...
def build_embeddings(opt, word_field, feat_fields, for_encoder=True):
    ...
    feat_embeddings_dims = []
    
    if for_encoder and (len(opt.numerical_feat_names) > 0):
        # checking if all given feature indexes are valid
        for j in opt.numerical_feat_names:
            if j >= len(feat_fields) or j < 0:
                opt.numerical_feat_names = []
                break
        # creating dimensions vector    
        if len(opt.numerical_feat_names) > 0:
            feat_embeddings_dims = [ (2 if (i in opt.numerical_feat_names) else int(vocab ** opt.feat_vec_exponent)) for i, vocab in enumerate(num_feat_embeddings)]
    ...
    emb = Embeddings(
        ...
        feat_embeddings_dims=feat_embeddings_dims
    )
    return emb
...
def build_base_model(model_opt, fields, gpu, checkpoint=None):
    ...
    if checkpoint is not None:
        ...
        if hasattr(model.encoder, 'embeddings'):
            ...
            model.encoder.embeddings.create_custom_features_embeddings(fields, model_opt.numerical_feat_names)            
